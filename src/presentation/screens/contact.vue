  ...
  <div class="flex flex-col min-h-screen bg-surface">
    ...
    <section class="py-20 bg-gradient-to-br from-primary-container to-primary text-white">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <h1 class="text-4xl md:text-5xl font-display font-bold mb-6">{{ pageData?.header?.heading ?? 'Partner With Us' }}</h1>
    ...
    <section class="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex-grow">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div class="bg-surface-container-lowest p-8 md:p-12 rounded-2xl ambient-shadow">
          ...
          <p class="text-on-surface-variant font-body mb-8">{{ pageData?.formSection?.body ?? 'Our PA5 Consultancy team aims to respond to all inquiries within 48 hours.' }}</p>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <button type="submit" :disabled="status === 'sending'" class="w-full py-4 bg-primary text-white font-bold rounded-lg hover:bg-primary-container transition-all flex justify-center items-center gap-2 group disabled:opacity-60">
              <span v-if="status === ''">{{ pageData?.formSection?.submitLabel ?? 'Submit Inquiry' }}</span>
              <span v-else-if="status === 'sending'" class="animate-pulse">Sending...</span>
              <span v-else-if="status === 'success'" class="text-leaf-300">{{ pageData?.formSection?.successMessage ?? 'Message Sent Successfully!' }}</span>
        ...
        <div class="flex flex-col justify-center space-y-12">
          <div>
            <h3 class="text-sm uppercase tracking-widest font-bold text-on-surface-variant mb-6">{{ pageData?.hqSection?.eyebrow ?? 'Global Headquarters' }}</h3>
          ...
          <div>
            <h3 class="text-sm uppercase tracking-widest font-bold text-on-surface-variant mb-6">{{ pageData?.directContacts?.eyebrow ?? 'Direct Contacts' }}</h3>
            <div class="space-y-6">
              <template v-if="departments.length > 0">
                <div v-for="dept in departments" :key="dept.label" class="flex items-center gap-4">
                  ...
                  <div>
                    <p class="font-bold text-on-surface">{{ dept.label }}</p>
...
type ContactPageGlobal = {
  header?: { heading?: string; body?: string }
  formSection?: {
    heading?: string
    body?: string
    interestAreas?: { value: string }[]
    submitLabel?: string
  ...
  directContacts?: {
    eyebrow?: string
    contacts?: { label: string; email: string; icon?: string }[]
  }
...
const { data: rawPage } = useAsyncData('contact-page-global', () =>
...
const defaultInterest = computed(() =>
  interestOptions.value[0] ?? 'Project Acquisition (PA1)'
)
